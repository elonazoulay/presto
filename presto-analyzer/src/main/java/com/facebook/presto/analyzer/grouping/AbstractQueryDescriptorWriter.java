...
{
    private static final Logger log = Logger.get(AbstractQueryDescriptorWriter.class);
    ...
    {
        return config.getPrefix().isEmpty() ?
                format("%s.%s", getMatchedSource(descriptor), getFileSuffix()) :
                format("%s.%s.%s", config.getPrefix(), getMatchedSource(descriptor), getFileSuffix());
    }
